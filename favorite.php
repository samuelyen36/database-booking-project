...
if ($conn->connect_error) {
    ...
    echo '<a href="user_page.php?order=5">HomePage</a>       <a href="logout.php">登出</a>       <br>';
    $login_id=$_SESSION['id'];
    $this_user=$_SESSION['id'];
    $select_favorite = "select house_id from favorite where user_id = $this_user ";

    $sql_house= "SELECT * FROM house inner join owner on house.owner_id=owner.owner_id where house.id = owner.house_id and house.id in ($select_favorite) ";
...
while($row_house = @mysqli_fetch_row($result_house)){
    if($row_house === 0){   //no houses
        echo "<br><br>there is no house in your favorite";
        break;
    }
  ...
  ?><tr><th>   <?php   echo "$row_house[0]";   //id  ?>  </th>
     ...
     ?></th>
         ...
         <th>  <a href="delete_favorite.php?house_id=<?php echo$row_house[0]?>&&userid=<?php echo$this_user?>"> delete from favorite </a> <br> </th>
        </tr><?php
        $flag=1;
    }
}
