...
$accountErr = $emailErr = $passwordErr = $conpasswordErr= $userErr=  "";
$account = $email = $password= $conpassword = $user="";
$ori_acc="";
$Errmsg=0;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["account"])) {
    ...
    // check if name only contains letters
      if (preg_match("/\s/",$account)) {    //no space is allowed
...
if($row[0]==$ori_acc){
  ...
  $sql="INSERT INTO `user_table`(account,password,name,email,auth) VALUES(?,?,?,?,?)";
	$pre = $conn->prepare($sql);
	$md5_password = md5($password);
  	$pre->bind_param("sssss",$account,$md5_password,$user,$email,$auth);
...
<br> Already have an account? click <a href="login.php">here</a> to login
